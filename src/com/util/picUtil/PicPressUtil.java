...
public class PicPressUtil {
	...
	private static final Logger logger = LoggerFactory.getLogger(PicPressUtil.class);
	/*** 
	 * 对图片进行操作 
	 * 
	 ...
	 * 
	 */
	  /*private static ImageUtil imageHelper = null; 
	  
	  public static ImageUtil getImageHelper() { 
	    if (imageHelper == null) { 
	    	imageHelper = new ImageUtil(); 
	    } 
	    return imageHelper; 
	  } */
	   ...
	   * 	  上传图片类型如： jpg
	   * 
	   * @return  InputStream 返回压缩后图片流
	   */
	  public static InputStream scaleImage(File sourcefile, double scale, String format) { 
	    ...
	    try { 
	      ...
	      ImageIO.write(outputImage, format, imOut);  
	      
	      inputStream = new ByteArrayInputStream(bs.toByteArray());  
	      
	    } catch (IOException e) { 
	      logger.error("scaleImage error:"  + e); 
	    } finally{
	    	try {  
	    	    if (bs != null) {  
	    	      bs.close();  
	    	    ...
	    	    logger.error("close scaleImage stream error:",e);  
	    	  }  
	    }
		return inputStream; 
