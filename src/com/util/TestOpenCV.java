...
public class TestOpenCV {
     ...
     static	String test_file_path = "D:\\soft" + File.separator + "testFiles";
	...
	static {
		 ...
		 * loadLibrary(Core.NATIVE_LIBRARY_NAME); //使用这种方式加载，需要在 IDE 中配置参数. Eclipse
		 * 配置：http://opencv-java-tutorials.readthedocs.io/en/latest/01-installing-opencv
		 * -for-java.html#set-up-opencv-for-java-in-eclipse IDEA 配置
		 * ：http://opencv-java-tutorials.readthedocs.io/en/latest/01-installing-opencv-
		 * for-java.html#set-up-opencv-for-java-in-other-ides-experimental
		 *
		 * 第二种方式 --------------System.load(path of lib); System.load(your path of lib)
		 * ,方式比较灵活，可根据环境的系统，位数，决定加载内容
		 */
		loadLibraries();
	}
	...
	public static void readTable() {
		...
		Mat source_image = Imgcodecs.imread("D:\\document\\fapiao\\pupiao\\0001.png");
		// 灰度处理
		...
		Imgproc.adaptiveThreshold(gray_image, thresh_image, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY,
				7, -2);
		saveImage("out-table/1-thresh.png", thresh_image);
		...
		/*
		 * 求水平线 1. 根据页面的列数（可以理解为宽度），将页面化成若干的扫描区域 2. 根据扫描区域的宽度，创建一根水平线 3.
		 * 通过腐蚀、膨胀，将满足条件的区域，用水平线勾画出来
		 *
		 * scale 越大，识别的线越多，因为，越大，页面划定的区域越小，在腐蚀后，多行文字会形成一个块，那么就会有一条线 在识别表格时，我们可以理解线是从页面左边
		 * 到 页面右边的，那么划定的区域越小，满足的条件越少，线条也更准确
		 */
		int scale = 10;
		 ...
		 * vector，这里将使用找到的轮廓的列表进行填充（即，这将是一个contours的vector,其中contours[i]表示一个特定的轮廓，这样，
		 * contours[i][j]将表示contour[i]的一个特定的端点）。
		 * 第三个参数，hierarchy，这个参数可以指定，也可以不指定。如果指定的话，输出hierarchy，将会描述输出轮廓树的结构信息。0号元素表示下一个轮廓
		 * （同一层级）；1号元素表示前一个轮廓（同一层级）；2号元素表示第一个子轮廓（下一层级）；3号元素表示父轮廓（上一层级）
		 * 第四个参数，轮廓的模式，将会告诉OpenCV你想用何种方式来对轮廓进行提取，有四个可选的值： CV_RETR_EXTERNAL
		...
		Imgproc.findContours(mask_image, contours, hierarchy, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE,
				new Point(0, 0));
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			/*
			 * 获取区域的面积 第一个参数，InputArray contour：输入的点，一般是图像的轮廓点 第二个参数，bool oriented =
			 ...
			 *
			 * MatOfPoint2f curve：像素点的数组数据。 MatOfPoint2f approxCurve：输出像素点转换后数组数据。 double
			 * epsilon：判断点到相对应的line segment
	...
	private static void saveImage(String path, Mat image) {
		...
		String outPath = test_file_path + File.separator + path;
		...
		try {
			...
			if (osName.startsWith("Windows")) {
				...
				// 64位系统
					...
					opencvpath = opencvpath + "C:\\Users\\gaoyakun\\Downloads\\opencv\\build\\java\\x64\\opencv_java330.dll";
				} else {
					opencvpath = opencvpath + "\\opencv\\x86\\Your path to .dll";
				}
			}
